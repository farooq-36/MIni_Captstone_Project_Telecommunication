...
CREATE TABLE `telos_chum`.`customer_provideds` (
  ...
  select * from customer_provideds;
    ...
    CREATE TABLE `telos_chum`.`location` 
   (
   `CustomerID` VARCHAR(45) NULL,
...
CREATE TABLE `telos_chum`.`service_provideds` (
...
select * from service_provideds;
  ...
  CREATE TABLE `telos_chum`.`payments_charges` (
  `CustomerID` VARCHAR(45) NULL,
...
CREATE TABLE `telo_chum`.`churnstatus` (
  `CustomerID` VARCHAR(45) NULL,
  ...
  insert into churnstatus values 
...
#Analysis the dataset and created different tables 
#Add the primary key column  because column should be uniquely  each record in a table and customerid as primary key
#foreign key is customerid  refers to the PRIMARY KEY in another table   and the table with the primary key is called the referenced or parent table and primary key is column customerid 
